...
void checkGoal()
{
    ...
    if (ball_y > field_height/2 && ball_x > -goal_width/2 && ball_x < goal_width/2 && ball_z < goal_height) {
        ...
        ball_x = 0; ball_y = 0; ball_z = ball_radius;
        ball_vx = 0; ball_vy = 0; ball_vz = 0;
    }
...
void updateBall()
{
    ...
    // Colisao com as bordas do campo (exceto os gols)
    ...
    // Colisao com as bordas laterais dos gols
    if ((ball_y > field_height/2 - ball_radius && (ball_x < -goal_width/2 || ball_x > goal_width/2)) ||
        (ball_y < -field_height/2 + ball_radius && (ball_x < -goal_width/2 || ball_x > goal_width/2))) {
        ball_y = (ball_y > 0) ? field_height/2 - ball_radius : -field_height/2 + ball_radius;
...
void processKeys()
{
    ...
    if (keys[' ']) { // Espaco para chutar para cima
        if (ball_z <= ball_radius + 0.1f) { // So pode chutar se estiver no chao
            ball_vz += 2.0f;
        }
    }
...
int main(int argc, char** argv)
{
    ...
    printf("Controles:\n");
    printf("WASD - Mover a bola\n");
    printf("ESPACO - Chutar para cima\n");
